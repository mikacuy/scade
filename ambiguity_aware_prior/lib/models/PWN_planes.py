...
class PWNPlanesLoss(nn.Module):
    ...
    def transfer_uvz(self, depth):
        max_uv = self.u_u0.max().to(depth.device)
        u = self.u_u0.repeat((depth.shape[0], 1, 1, 1)).to(depth.device) / max_uv
        v = self.v_v0.repeat((depth.shape[0], 1, 1, 1)).to(depth.device) / max_uv
        z = depth
        pw = torch.cat([u, v, z], 1).permute(0, 2, 3, 1) # [b, h, w, c]
        return pw

    def transfer_xyz(self, depth):
        x = self.u_u0.to(depth.device) * torch.abs(depth) / self.fx.to(depth.device)
        y = self.v_v0.to(depth.device) * torch.abs(depth) / self.fy.to(depth.device)
        z = depth
    ...
    def forward(self, gt_depth, pred_depth, mask, focal_length=None):
        ...
        loss = torch.tensor(0.0).unsqueeze(0).repeat(B).to(pred_depth.device)
        valid_planes_num = 0
        for i in range(B):
