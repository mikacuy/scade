...
class MSGIL_NORM_Loss(nn.Module):
    ...
    def one_scale_gradient_loss(self, pred_scale, gt, mask):
        ...
        valid_num = torch.mean(h_mask.view(batch_size, -1), axis=-1) + torch.mean(v_mask.view(batch_size, -1), axis=-1)

        gradient_loss = torch.mean(h_gradient.view(batch_size, -1), axis=-1) + torch.mean(v_gradient.view(batch_size, -1), axis=-1)
    ...
    def forward(self, pred, gt):
        ...
        B = gt.shape[0]
        grad_term = torch.tensor(0.0).unsqueeze(0).repeat(B).to(pred.device)
